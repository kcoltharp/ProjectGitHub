 ...
 */
public class OpenShiftInstanceManager {
    ...
    private static OpenShiftInstanceManager instance;
    private List<OpenShiftInstance> instances = new ArrayList<OpenShiftInstance>();
    private ChangeSupport listeners;
    private static final Logger LOG = Logger.getLogger(OpenShiftInstanceManager.class.getSimpleName());
    public static synchronized OpenShiftInstanceManager getDefault() {
        if (instance == null) {
            instance = new OpenShiftInstanceManager();
        }
        return instance;
    }
    public OpenShiftInstanceManager() {
    ...
    public List<OpenShiftInstance> getInstances() {
        return instances;
    }
    public void add(OpenShiftInstance ai) {
        store(ai);
        instances.add(ai);
        notifyChange();
    }
    private void store(OpenShiftInstance ai) {
        InstanceProperties props = InstancePropertiesManager.getInstance().createProperties(AMAZON_IP_NAMESPACE);
//        Keyring.save(PREFIX + KEY_ID + "." + ai.getName(), ai.getKeyId(), "Amazon Access Key ID"); // NOI18N
...
//        props.putString("region", ai.getRegionURL()); // NOI18N
    ...
    private static List<OpenShiftInstance> load() {
        List<OpenShiftInstance> result = new ArrayList<OpenShiftInstance>();
...
//            result.add(new AmazonInstance(name, keyId, key, region));
    ...
    void remove(OpenShiftInstance ai) {
        for (InstanceProperties props : InstancePropertiesManager.getInstance().getProperties(AMAZON_IP_NAMESPACE)) {
